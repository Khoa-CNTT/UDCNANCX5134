    ...
    for idx, det in enumerate(detections):
        ...
        if label == "motorcyclist" and confidence >= 0.7:
            ...
            for obj in detections:
                if obj["name"] == "helmet" and obj["confidence"] >= 0.05:
                    helmet_xmin, helmet_ymin = int(obj["xmin"]), int(obj["ymin"])
                    helmet_xmax, helmet_ymax = int(obj["xmax"]), int(obj["ymax"])
                    if xmin <= helmet_xmin and xmax >= helmet_xmax and ymin <= helmet_ymin and ymax >= helmet_ymax:
                        helmet_detected = True
                        break  # Nếu đã phát hiện đội mũ thì bỏ qua motorcyclist này
            ...
            if not helmet_detected:
                ...
                # Kiểm tra nếu có biển số trong phạm vi motorcyclist
                ...
                # Trả kết quả về
                output.append({
                    "motorcyclist_folder": "http://127.0.0.1:5000/" + motorcyclist_folder.replace("\\", "/"),
                    "motorcyclist_img_path": "http://127.0.0.1:5000/" + motorcyclist_img_path.replace("\\", "/"),
                    "license_plate_path": "http://127.0.0.1:5000/" + license_plate_path.replace("\\", "/") if license_plate_img is not None else False,
